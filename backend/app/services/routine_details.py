...
class Routine_Detail_Services:
    ...
    # 루틴 디테일 생성
    async def services_routine_details_create(
    db: AsyncSession,
    data: Routine_Detail_Create
        ...
        try:
            ...
            new_routine_detail = await Routine_Detail_CRUD.crud_routine_details_create(
                db, data_dict
            )
    ...
    async def services_routine_details_read_by_id(db:AsyncSession, r_id : int,u_id: int):
        read_routine_detail = await Routine_Detail_CRUD.crud_routine_details_read_by_id(db, r_id)

        if not read_routine_detail :
            return []
        
        if read_routine_detail[0].routine.u_id != u_id:
    ...
    async def service_routine_detail_read_by_r_d_id(
        ...
        if not read_routine_detail:
            return []
        if read_routine_detail.routine.u_id != u_id:
            raise HTTPException(403, "권한 없음")
        return read_routine_detail

    # 루틴 디테일 아이디 기반 업데이트
    async def services_routine_details_update(
        db: AsyncSession,
        r_d_id: int,
        ...
        try:
            ...
            if "weight" in data_dict and data_dict["weight"] is not None:
                data_dict["weight"] = int(data_dict["weight"])
