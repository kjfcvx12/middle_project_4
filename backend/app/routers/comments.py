...
async def routers_comments_create(b_id:int, comment:CommentCreate, db:AsyncSession=Depends(get_db), u_id:int = Depends(auth_get_u_id)):
...
async def routers_comments_update(b_id:int, c_id:int,comment_data:CommentUpdate, db:AsyncSession = Depends(get_db),u_id:int = Depends(auth_get_u_id)):
    await CommentService.services_comments_update(db, b_id, c_id, comment_data, u_id)
    return {"msg":"수정완료"}

#댓글 삭제
@router.delete("/{b_id}/comments/{c_id}")
async def routers_comments_delete(b_id:int, c_id:int, db:AsyncSession = Depends(get_db), u_id:int = Depends(auth_get_u_id)):
