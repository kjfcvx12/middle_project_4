...
import { routine_detail_create, routine_detail_read_all,
    routine_detail_delete, routine_detail_update
...
const Routine_details = () => {
    ...
    const move_up = async (index) => {
        ...
        await routine_detail_update(current.r_d_id, {step : prev.step});
        await routine_detail_update(prev.r_d_id, {step : current.step});
    ...
    const move_down = async (index) => {
        ...
        await routine_detail_update(current.r_d_id, {step : next.step});
        await routine_detail_update( next.r_d_id, {step : current.step});
    ...
    <div>
        ...
        {list.map((ex,i)=>(
            <div key={ex.r_d_id}>
                ...
                {/* 수정 */}
                {editId === ex.r_d_id && editForm.r_d_id === ex.r_d_id ? (
                    <div>
                        <input value={editForm.sets ?? ''}
                        onChange={(e)=>setEditForm({...editForm, sets:e.target.value})} />

                        <input value={editForm.reps ?? ''}
                        onChange={(e)=>setEditForm({...editForm, reps:e.target.value})} />

                        <input value={editForm.weight ?? ''}
                        onChange={(e)=>setEditForm({...editForm, weight:e.target.value})} />

                        <input value={editForm.rest_time ?? ''}
                        onChange={(e)=>setEditForm({...editForm, rest_time:e.target.value})} />

                        <button onClick={handle_update}>저장</button>
                    </div>
                ) :(
                    <p>{ex.sets}세트 x {ex.reps}회 x {ex.weight ?? 0}kg</p>
                )}
                ...
                <button onClick={()=>handle_delete(ex.r_d_id)}>삭제</button>

                <button onClick={()=>move_up(i)}>세트 위로 올리기</button>
                <button onClick={()=>move_down(i)}>세트 아래로 내리기</button>
            ...
            <h3>운동 루틴 추가</h3>
                ...
                순서 : <input name="step" placeholder="순서" value={form.step} onChange={handle_change} />
                <br />
                세트 수 :<input name="sets" placeholder="세트" value={form.sets} onChange={handle_change} />
                <br />
                반복 횟수 :<input name="reps" placeholder="횟수" value={form.reps} onChange={handle_change} />
                <br />
                휴식 시간 :<input name="rest_time" placeholder="휴식" value={form.rest_time} onChange={handle_change} />
                <br />
                중량 :<input name="weight" placeholder="무게" value={form.weight} onChange={handle_change} />
                <br />
